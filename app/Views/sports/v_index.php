...
<div class="container-fluid card">
  <div class="card-header mb-4 pb-0 d-flex align-items-center justify-content-between">
    <h4>Sports List</h4>
    <a href="<?= base_url('/sports/form') ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Add User</a>
  </div>
          ...
          <?php else: ?>
            <?php foreach ($sports as $sport): ?>
              <tr>
                ...
                <td><?= $sport['slug'] ?></td>
                <td>
                  <a href="<?= base_url('/sports/form?id=' . $sport['sport_id']) ?>" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                  <form action="<?= base_url('/sports/delete/' . $sport['sport_id']) ?>" method="post" style="display:inline-block;">
                    <?= csrf_field() ?>
...
<script type="text/javascript">
  ...
  function handlePagination(pageNumber) {
    window.location.replace(`<?php echo base_url(); ?>sports?page=${pageNumber}`);
  }
